...
import BulkOrderForm from "./components/Order/Customer/BulkOrderForm";
...
import AIDesign from "./components/Product/AIDesign/AIDesign";
import DesignLab from "./components/Product/DesignLab/DesignLab";
import Header from "./components/Basics/Header";
import Footer from "./components/Basics/Footer";
import FashionChatbot from "./components/Basics/FashionChatbot";
...
function PrivateRoute({ children, roles, privileges }) {
  ...
  if (!isAdmin && !roleAllowed && !hasPrivilege) {
    return <Navigate to="/" replace />;
  }

  if (!isAdmin && privileges?.length && !hasPrivilege && allowedRoles.length === 0) {
    return <Navigate to="/" replace />;
  }
...
function App() {
  ...
  const isMarketplace = currentPath === "/customer-products";
  const isFlush = location.pathname === "/" || isAuthRoute || isDashboardRoute || isMarketplace;
    ...
    <AuthProvider>
      ...
      <AdminCartProvider>
        ...
        <CartProvider>
          ...
          <div
            className={`content ${isFlush ? "content--flush" : ""} ${isAuthRoute ? "content--auth" : ""
              } ${isMarketplace ? "content--marketplace" : ""}`}
          >
            <Routes>
              ...
              <Route path="/ai-design" element={
                <PrivateRoute roles={["user", "admin"]}><AIDesign /></PrivateRoute>
              } />
              <Route path="/design-lab" element={
                <PrivateRoute roles={["user", "admin"]}><DesignLab /></PrivateRoute>
              } />
              ...
              <Route path="/bulk-orders" element={
                <PrivateRoute roles={["user", "admin"]}><BulkOrderForm /></PrivateRoute>
              } />
