  ...
  return (
    <div style={{ position: "relative", width: size, height: size }}>
      ...
      >
        ...
        <span style={{ fontSize: 12, color: "var(--text-muted, #64748b)" }}>Total</span>
      </div>
    </div>
  );
}
        ...
        {values.map((v, i) => {
          ...
          return (
            <g key={i}>
              ...
              <text x={x + barWidth / 2} y={y - 6} fontSize={11} fill="currentColor" textAnchor="middle">
                {v ? v.toFixed(2) : ""}
              </text>
      ...
      try {
        ...
        try {
          ...
          if (pRes.ok) {
            ...
            setProducts(list);
            if (list.length) {
              setSelectedProduct((prev) => prev || list[0]._id || list[0].productId || list[0].id || "");
            }
          }
        } catch {
