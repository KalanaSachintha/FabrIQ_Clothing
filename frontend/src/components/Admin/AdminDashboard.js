...
import { UserPlus, Plus, Sun, Moon, Download } from "lucide-react";
...
import AdminBulkOrders from "../Order/Admin/AdminBulkOrders";
import SalesAnalytics from "./SalesAnalytics";
import CustomerLoyalty from "./CustomerLoyalty";
import ForecastWidget from "./ForecastWidget";
import "./AdminDashboard.css";

const CustomerDiscountAdminList = React.lazy(() => import("../SupplierProduct/CustomerDiscountAdminList"));
...
const PRIV_LABELS = {
  ...
  sales_refund: "Legacy: sales refund console",
...
const REORDER_STORAGE_KEY = "fabriq_reorder_queue";

// Predefined apparel sizes exposed in the inventory editor
const SIZE_OPTIONS = ["XS", "S", "M", "L", "XL", "XXL"];

const makeVariantKey = (variant, index) => {
  const label = typeof variant?.colorName === "string" ? variant.colorName.trim().toLowerCase() : "";
  return `${index}-${label || "variant"}`;
};
...
const orderSizes = (sizes) => {
  ...
  const standard = SIZE_OPTIONS.filter((size) => seen.has(size));
  const extras = cleaned.filter((size) => !SIZE_OPTIONS.includes(size));
...
const toSizeStockMap = (input) => {
  ...
  if (Array.isArray(input)) {
    input.forEach((item) => {
      ...
      if (!label) return;
      const stock = Number(item.stockAmount ?? item.stock ?? 0);
      const normalizedStock = Number.isFinite(stock) && stock > 0 ? Math.floor(stock) : Math.max(0, Math.floor(stock || 0));
      map[label] = String(normalizedStock);
    });
    return map;
  }
  ...
  if (typeof input === "object") {
    Object.entries(input).forEach(([key, value]) => {
      const label = typeof key === "string" ? key.trim().toUpperCase() : "";
      if (!label) return;
...
const cloneSizeStockMap = (map) => {
  if (!map || typeof map !== "object") return {};
  return Object.fromEntries(Object.entries(map).map(([size, stock]) => [size, String(stock)]));
};

const sizeStockMapsEqual = (a, b) => {
  const mapA = a || {};
  const mapB = b || {};
  ...
  for (const key of keysA) {
    if (!Object.prototype.hasOwnProperty.call(mapB, key)) return false;
    if (String(mapA[key]) !== String(mapB[key])) return false;
  }
  return true;
};

const calculateSizeStockTotal = (map) => {
...
const computeVariantDirty = (entries, baseline) => {
  if (!Array.isArray(entries) || !Array.isArray(baseline)) return false;
  if (entries.length !== baseline.length) return true;
  for (let i = 0; i < entries.length; i += 1) {
    const entry = entries[i];
    const base = baseline[i];
    if (!entry || !base) return true;
    const stockSame = entry.stockAmount === base.stockAmount;
    const sizesSame = arraysEqual(entry.availableSizes, base.availableSizes);
    const mapsSame = sizeStockMapsEqual(entry.sizeStockMap, base.sizeStockMap);
    if (!stockSame || !sizesSame || !mapsSame) {
      return true;
    }
  }
  return false;
...
export default function AdminDashboard() {
  ...
  const [variantCustomSizeDrafts, setVariantCustomSizeDrafts] = useState({});
  ...
  useEffect(() => {
    ...
    setVariantDrafts(() => {
      ...
      products.forEach((product) => {
        ...
        const entries = variants.map((variant, index) => {
          ...
          const variantSizes = Array.isArray(variant?.availableSizes)
            ? variant.availableSizes
                .map((value) => (typeof value === "string" ? value.trim().toUpperCase() : ""))
                .filter(Boolean)
            : [];
          const combinedSizes = orderSizes([...new Set([...variantSizes, ...sizeKeys])]);
          const colorStock = sizeKeys.length
            ? calculateSizeStockTotal(sizeStockMap)
            : Math.max(0, Math.floor(Number(variant?.stockAmount ?? 0) || 0));
          return {
            key: makeVariantKey(variant, index),
            colorName: variant?.colorName || `Colour ${index + 1}`,
            stockAmount: String(colorStock),
        ...
        const baseline = entries.map((entry) => ({
          ...
          sizeStockMap: cloneSizeStockMap(entry.sizeStockMap),
        }));
        nextDrafts[product._id] = { entries, baseline, dirty: false };
      });
      return nextDrafts;
    });
    ...
    setVariantCustomSizeDrafts(() => {
      ...
      products.forEach((product) => {
        const variants = Array.isArray(product?.colorVariants) ? product.colorVariants : [];
        next[product._id] = variants.map(() => ({ size: "", stock: "" }));
      });
      return next;
    });
  }, [products]);
  ...
  const handleVariantStockChange = (productId, variantIndex, rawValue) => {
    setVariantDrafts((prev) => {
      const group = prev[productId];
      if (!group) return prev;
      const entries = group.entries.map((entry, idx) => {
        if (idx !== variantIndex) return entry;
        if (Object.keys(entry.sizeStockMap || {}).length) {
          return entry;
        }
        let nextValue = entry.stockAmount;
        if (typeof rawValue === "string") {
          if (rawValue.trim() === "") {
            ...
            if (Number.isFinite(numeric)) {
              nextValue = String(Math.max(0, Math.floor(numeric)));
            } else {
              nextValue = rawValue;
            }
          }
        } else if (typeof rawValue === "number") {
          nextValue = String(Math.max(0, Math.floor(rawValue)));
        }
        return {
          ...entry,
          stockAmount: nextValue,
        };
      });
      return {
        ...prev,
        [productId]: {
          ...group,
          entries,
          dirty: computeVariantDirty(entries, group.baseline),
        },
      };
    });
  };

  const handleVariantSizeToggle = (productId, variantIndex, size) => {
    ...
    setVariantDrafts((prev) => {
      ...
      const entries = group.entries.map((entry, idx) => {
        if (idx !== variantIndex) return entry;
        const current = Array.isArray(entry.availableSizes) ? entry.availableSizes : [];
        const set = new Set(
          current
            .map((value) => (typeof value === "string" ? value.trim().toUpperCase() : ""))
            .filter(Boolean)
        );
        const map = cloneSizeStockMap(entry.sizeStockMap);
        ...
        return {
          ...entry,
          availableSizes: orderSizes(Array.from(set)),
          sizeStockMap: map,
          stockAmount: String(calculateSizeStockTotal(map)),
        };
      });
      return {
        ...prev,
  ...
  const handleVariantSizeStockChange = (productId, variantIndex, size, value) => {
    const normalized = typeof size === "string" ? size.trim().toUpperCase() : "";
    if (!normalized) return;
    setVariantDrafts((prev) => {
      const group = prev[productId];
      ...
      const entries = group.entries.map((entry, idx) => {
        ...
        if (typeof value === "string") {
          if (value.trim() === "") {
            ...
            const numeric = Number(value);
            if (Number.isFinite(numeric)) {
              map[normalized] = String(Math.max(0, Math.floor(numeric)));
            } else {
              map[normalized] = value;
            }
          }
        } else if (typeof value === "number") {
          map[normalized] = String(Math.max(0, Math.floor(value)));
        }
        const total = calculateSizeStockTotal(map);
        const sizeSet = new Set(
          (entry.availableSizes || [])
            .map((val) => (typeof val === "string" ? val.trim().toUpperCase() : ""))
            .filter(Boolean)
        );
        sizeSet.add(normalized);
        return {
          ...entry,
          availableSizes: orderSizes(Array.from(sizeSet)),
          sizeStockMap: map,
          stockAmount: String(total),
        };
      });
      return {
        ...prev,
  ...
  const handleVariantCustomSizeInputChange = (productId, variantIndex, field, value) => {
    setVariantCustomSizeDrafts((prev) => {
      const list = Array.isArray(prev[productId]) ? [...prev[productId]] : [];
      while (list.length <= variantIndex) {
        list.push({ size: "", stock: "" });
      }
      const current = { ...(list[variantIndex] || { size: "", stock: "" }) };
      if (field === "size") {
        current.size = typeof value === "string" ? value.toUpperCase() : "";
      } else if (field === "stock") {
        if (typeof value === "string") {
          current.stock = value.replace(/[^0-9]/g, "");
        } else {
          current.stock = String(value ?? "");
        }
      }
      list[variantIndex] = current;
      return {
        ...prev,
        [productId]: list,
      };
  ...
  const handleVariantCustomSizeAdd = (productId, variantIndex) => {
    const drafts = variantCustomSizeDrafts[productId] || [];
    const draft = drafts[variantIndex] || { size: "", stock: "" };
    const normalizedSize = (draft.size || "").trim().toUpperCase();
    if (!normalizedSize) {
      setErr("Enter a size label before adding");
      return;
    }
    const numericValue = Number(draft.stock);
    if (!Number.isFinite(numericValue) || numericValue < 0) {
      setErr("Enter a non-negative stock value for the new size");
      return;
    }
    const stockString = String(Math.max(0, Math.floor(numericValue)));
    setVariantDrafts((prev) => {
      const group = prev[productId];
      if (!group) return prev;
      const entries = group.entries.map((entry, idx) => {
        if (idx !== variantIndex) return entry;
        const map = cloneSizeStockMap(entry.sizeStockMap);
        map[normalizedSize] = stockString;
        const sizeSet = new Set(
          (entry.availableSizes || [])
            .map((value) => (typeof value === "string" ? value.trim().toUpperCase() : ""))
            .filter(Boolean)
  ...
  const handleVariantRemoveCustomSize = (productId, variantIndex, size) => {
    const normalized = typeof size === "string" ? size.trim().toUpperCase() : "";
    if (!normalized) return;
    setVariantDrafts((prev) => {
      const group = prev[productId];
  ...
  const handleVariantReset = (productId) => {
    setVariantDrafts((prev) => {
      const group = prev[productId];
      if (!group) return prev;
      const entries = group.entries.map((entry, idx) => {
        const base = group.baseline?.[idx];
        if (!base) return entry;
        const baseMap = cloneSizeStockMap(base.sizeStockMap);
        return {
          ...entry,
          stockAmount: base.stockAmount ?? String(calculateSizeStockTotal(baseMap)),
          availableSizes: [...base.availableSizes],
          sizeStockMap: baseMap,
        };
      });
      return {
        ...prev,
    ...
    setVariantCustomSizeDrafts((prev) => {
      ...
      return {
        ...prev,
        [productId]: current.map(() => ({ size: "", stock: "" })),
      };
    });
  };

  const handleSaveVariantGroup = async (productId) => {
    ...
    const draftGroup = variantDrafts[productId];
    if (!draftGroup || !Array.isArray(draftGroup.entries) || draftGroup.entries.length === 0) {
      setErr("No colour variants to update");
      return;
    }
    ...
    for (let idx = 0; idx < draftGroup.entries.length; idx += 1) {
      const entry = draftGroup.entries[idx];
      if (!entry) continue;
      const colorName = entry.colorName || product.colorVariants?.[idx]?.colorName || `Colour ${idx + 1}`;
      const map = entry.sizeStockMap || {};
      const sizeStockEntries = [];
      let summedSizeStock = 0;
      Object.entries(map).forEach(([sizeKey, rawStock]) => {
        const sizeLabel = typeof sizeKey === "string" ? sizeKey.trim().toUpperCase() : "";
        if (!sizeLabel) return;
        const numeric = Number(rawStock);
        if (!Number.isFinite(numeric) || numeric < 0) {
          sizeStockEntries.push({ error: sizeLabel });
          return;
        }
        const normalizedStock = Math.max(0, Math.floor(numeric));
        sizeStockEntries.push({ size: sizeLabel, stockAmount: normalizedStock });
        summedSizeStock += normalizedStock;
      });

      const invalidSize = sizeStockEntries.find((item) => item.error);
      if (invalidSize) {
        setErr(`Enter a valid stock amount for size ${invalidSize.error} in ${colorName}`);
        return;
      }

      const hasSizeStocks = sizeStockEntries.length > 0;
      const fallbackNumeric = Number(entry.stockAmount);
      if (!hasSizeStocks && (!Number.isFinite(fallbackNumeric) || fallbackNumeric < 0)) {
        setErr(`Enter a valid stock amount for ${colorName}`);
        return;
      }

      const normalizedStock = hasSizeStocks
        ? summedSizeStock
        : Math.max(0, Math.floor(fallbackNumeric || 0));

      totalStock += normalizedStock;

      const sizes = Array.isArray(entry.availableSizes) ? entry.availableSizes : [];
      const normalizedSizes = orderSizes(
        [
          ...new Set(
            [
              ...sizes.map((value) => (typeof value === "string" ? value.trim().toUpperCase() : "")),
              ...sizeStockEntries.map((item) => item.size),
            ].filter(Boolean)
          ),
        ]
      );
      ...
      preparedVariants.push({
        ...
        existingImageUrls: Array.isArray(entry.imageUrls) ? entry.imageUrls : [],
        imageKeys: [],
      });
    }

    if (!preparedVariants.length) {
      setErr("Add at least one colour variant before saving");
      return;
    }
    ...
    try {
      ...
      setMsg(`Updated colour stock for ${updatedProduct.name || "product"}`);
    } catch (error) {
  ...
  const buildProductUpdatePayload = (product, overrides = {}) => {
    ...
    if (overrides.hasOwnProperty("colorVariants")) {
      payload.colorVariants = overrides.colorVariants;
    }
  ...
  const handleApplyExactStock = (productId) => {
    ...
    if (Array.isArray(product?.colorVariants) && product.colorVariants.length) {
      setErr("Manage stock per colour using the controls below.");
      return;
    }
    ...
    <div className="dashboard-shell dashboard-shell--two-column admin-dashboard">
      ...
      <aside className="card dashboard-sidebar">
        ...
        <nav className="dashboard-nav stack-sm">
          ...
          <button
            type="button"
            onClick={() => setTabAndHash("bulk-orders")}
            className={`sidebar-link ${tab === "bulk-orders" ? "is-active" : ""}`}
          >
            <span className="sidebar-link__icon">📦</span>
          ...
          <button
            type="button"
            onClick={() => setTabAndHash("forecast")}
            className={`sidebar-link ${tab === "forecast" ? "is-active" : ""}`}
          >
            <span className="sidebar-link__icon">📈</span>
          ...
          <section className="nav-section stack-xs">
            ...
            <button
              type="button"
              onClick={() => setTabAndHash("discounts")}
            ...
            <button
              type="button"
              onClick={() => setTabAndHash("customer-discounts")}
              className={`sidebar-link ${tab === "customer-discounts" ? "is-active" : ""}`}
            >
              <span className="sidebar-link__icon">🎁</span>
            ...
            <button
              type="button"
              onClick={() => setTabAndHash("customer-loyalty")}
              className={`sidebar-link ${tab === "customer-loyalty" ? "is-active" : ""}`}
            >
              <span className="sidebar-link__icon">🏆</span>
        ...
        <header className="card dashboard-header">
          ...
          <div className="dashboard-header__actions">
            <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
              <button type="button" className="btn btn-secondary theme-toggle-btn" onClick={toggleTheme}>
          ...
          <section className="card stack-md">
            <div className="card-heading">
              ...
              <div className="action-grid">
                ...
                >
                  <Download size={18} />
                  <span>{userReportLoading ? "Exporting…" : "Export report"}</span>
                </button>
                <button className="btn-create-user" onClick={openCreateUser}>
                  <UserPlus size={18} />
              ...
              <div className="table-scroller">
                <table className="inventory-table">
                  ...
                  <tbody>
                    {loadingProducts ? (
                      <tr>
                        <td colSpan={5} className="empty-cell">Refreshing inventory…</td>
                      </tr>
                      ...
                      filteredInventoryItems.map((product) => {
                        ...
                        const reminderDaysNumber = Number.isFinite(Number(product.expiryReminderDays))
                          ...
                          : 0;
                        const hasColourVariants = Array.isArray(product?.colorVariants) && product.colorVariants.length > 0;
                        const variantGroup = variantDrafts[product._id] || {};
                        const variantEntries = hasColourVariants && Array.isArray(variantGroup.entries)
                          ? variantGroup.entries
                          : [];
                        const customSizeDrafts = variantCustomSizeDrafts[product._id] || [];
                        const variantDirty = Boolean(variantGroup?.dirty);
                        
                        const isExpanded = expandedInventoryId === (product._id || product.id);

                        return (
                          <React.Fragment key={product._id || product.id}>
                            <tr 
                              className={`inventory-row ${isExpanded ? "is-expanded" : ""} ${stock <= lowStockLimit ? "row-low-stock" : ""}`}
                              onClick={() => setExpandedInventoryId(isExpanded ? null : (product._id || product.id))}
                            >
                              ...
                              <td className="product-info-cell">
                                <div className="flex items-center gap-3">
                                  ...
                                  <div className="stack-3xs">
                                    <div className="font-bold text-sm">{product?.name || "Unnamed product"}</div>
                                    <div className="muted-text text-xs">
                              ...
                              <td>
                                <div className="stack-3xs">
                                  <div className={`font-bold ${stock <= lowStockLimit ? "text-danger" : "text-success"}`}>
                                    {stock.toLocaleString()} units
                                  </div>
                                  {Number.isFinite(value) && (
                                    <div className="muted-text text-xs">{formatCurrency(value)} value</div>
                                  )}
                                </div>
                              </td>
                              <td>
                                {hasColourVariants ? (
                                  <div className="flex flex-wrap gap-1">
                                    {product.colorVariants.map((v, i) => (
                                      <span key={i} className="variant-pill" title={`${v.stockAmount} units`}>
                                        {v.colorName}
                                      </span>
                                    ))}
                                  </div>
                                ) : (
                                  <span className="muted-text text-xs italic">No variants</span>
                                )}
                              </td>
                              <td className="text-right">
                                <div className="flex items-center justify-end gap-2">
                                  {stock <= lowStockLimit && <span className="badge badge-amber badge-sm">Low Stock</span>}
                                  <button
                                    type="button"
                                    className={`btn btn-sm ${isExpanded ? "btn-secondary" : "btn-primary"}`}
                                    onClick={(e) => {
                                      e.stopPropagation();
                                      setExpandedInventoryId(isExpanded ? null : (product._id || product.id));
                                    }}
                                  >
                                ...
                                <td colSpan={5}>
                                  <div className="inventory-expanded-panel">
                                    <div className="inventory-expanded-grid">
                                      <div className="inventory-editor-container">
                                        <div className="section-title">
                                          <span>Stock Management & Expiry</span>
                                          {variantDirty && <span className="dirty-indicator">Unsaved changes</span>}
                                        </div>
                                        
                                        {hasColourVariants ? (
                                          <div className="variant-editor-stack">
                                            {variantEntries.map((entry, variantIndex) => {
                                              const preview = Array.isArray(entry.imageUrls) && entry.imageUrls.length
                                                ? resolveImageUrl(entry.imageUrls[0])
                                                : null;
                                              const sizeStockMap = entry.sizeStockMap || {};
                                              const sizeSelections = orderSizes([
                                                ...new Set([
                                                  ...(Array.isArray(entry.availableSizes) ? entry.availableSizes : []),
                                                  ...Object.keys(sizeStockMap),
                                                ]),
                                              ]);
                                              const customSizes = sizeSelections.filter((size) => !SIZE_OPTIONS.includes(size));
                                              const customDraft = customSizeDrafts[variantIndex] ?? { size: "", stock: "" };
                                              const savedStockValue = Number(product.colorVariants?.[variantIndex]?.stockAmount ?? 0);
                                              const disableColourStock = Object.keys(sizeStockMap).length > 0;
                                              const productKey = product._id || product.id || "product";
                                              
                                              return (
                                                <div key={entry.key || variantIndex} className="inventory-variant-card">
                                                  <div className="inventory-variant-card__header">
                                                    <div className="flex items-center gap-3">
                                                      {preview ? (
                                                        <img src={preview} alt="" className="v-thumb" onError={(e) => e.target.style.display="none"} />
                                                      ) : (
                                                        <div className="v-thumb-placeholder" />
                                                      )}
                                                      <div className="stack-3xs">
                                                        <div className="font-bold">{entry.colorName}</div>
                                                        <div className="text-xs muted-text">{savedStockValue.toLocaleString()} in stock</div>
                                                      </div>
                                                    </div>
                                                    <div className="v-stock-field">
                                                      <label className="text-xs uppercase font-bold text-muted">Manual Unit Stock</label>
                                                      <input
                                                        type="number"
                                                        className="input input-sm"
                                                        value={entry.stockAmount}
                                                        onChange={(e) => handleVariantStockChange(product._id, variantIndex, e.target.value)}
                                                        disabled={disableColourStock}
                                                      />
                                                      {disableColourStock && <div className="text-xxs text-muted mt-1">Controlled by size breakdown</div>}
                                                    </div>
                                                  </div>
                                                  
                                                  <div className="inventory-variant-card__sizes">
                                                    <div className="label-xs mb-2">Per-Size Stock Level</div>
                                                    <div className="size-grid-scroller">
                                                      {SIZE_OPTIONS.map((size) => {
                                                        const toggleId = `${productKey}-${variantIndex}-${size}`;
                                                        const checked = sizeSelections.includes(size);
                                                        const sizeValue = checked ? sizeStockMap[size] ?? "" : "";
                                                        return (
                                                          <div key={size} className={`size-item ${checked ? "is-active" : ""}`}>
                                                            <div className="flex items-center gap-2">
                                                              <input
                                                                id={toggleId}
                                                                type="checkbox"
                                                                checked={checked}
                                                                onChange={() => handleVariantSizeToggle(product._id, variantIndex, size)}
                                                              />
                                                              <label htmlFor={toggleId}>{size}</label>
                                                            </div>
                                                            <input
                                                              type="number"
                                                              ...
                                                              placeholder="0"
                                                              value={sizeValue}
                                                              onChange={(e) => handleVariantSizeStockChange(product._id, variantIndex, size, e.target.value)}
                                                              disabled={!checked}
                                                            />
                                                          </div>
                                                        );
                                                  ...
                                                  {(customSizes.length > 0 || customDraft.size || customDraft.stock) && (
                                                    <div className="inventory-variant-card__custom mt-3">
                                                      <div className="label-xs mb-1">Custom & Professional Sizing</div>
                                                      <div className="flex flex-wrap gap-2">
                                                        {customSizes.map((size) => (
                                                          <div key={size} className="custom-size-pill">
                                                            ...
                                                            <button 
                                                              ...
                                                              onClick={() => handleVariantRemoveCustomSize(product._id, variantIndex, size)}
                                                            >&times;</button>
                                                          </div>
                                                        ))}
                                                        <div className="custom-size-add-form">
                                                          <input 
                                                            type="text" 
                                                            placeholder="Label" 
                                                            className="input input-xs" 
                                                            value={customDraft.size}
                                                            onChange={(e) => handleVariantCustomSizeInputChange(product._id, variantIndex, "size", e.target.value)}
                                                          />
                                                          <input 
                                                            type="number" 
                                                            placeholder="Qty" 
                                                            className="input input-xs" 
                                                            value={customDraft.stock}
                                                            onChange={(e) => handleVariantCustomSizeInputChange(product._id, variantIndex, "stock", e.target.value)}
                                                          />
                                                          <button 
                                                            type="button" 
                                                            className="btn btn-primary btn-xs"
                                                            onClick={() => handleVariantCustomSizeAdd(product._id, variantIndex)}
                                                            disabled={!customDraft.size}
                                                          >Add</button>
                                                        </div>
                                                      </div>
                                                    </div>
                                          ...
                                          <div className="simple-stock-editor p-4 bg-muted-v rounded-xl border border-dashed border-muted">
                                            <p className="text-xs muted-text mb-4">No variants detected. Manage direct product stock below.</p>
                                            <div className="grid gap-4 sm:grid-cols-2">
                                              <div className="stack-3xs">
                                                <label className="text-xs uppercase font-bold text-muted">Direct Stock Level</label>
                                                <div className="flex gap-2">
                                                  <input
                                                    type="number"
                                                    className="input input-sm"
                                                    value={drafts.set ?? ""}
                                                    onChange={(e) => handleStockDraftChange(product._id, "set", e.target.value)}
                                                  ...
                                                  <button
                                                    ...
                                                    disabled={savingThisProduct || !drafts.set}
                                                  >Set</button>
                                                </div>
                                              </div>
                                              <div className="stack-3xs">
                                                <label className="text-xs uppercase font-bold text-muted">Stock Reduction</label>
                                                <div className="flex gap-2">
                                                  <input
                                                    type="number"
                                                    className="input input-sm"
                                                    value={drafts.reduce ?? ""}
                                                    onChange={(e) => handleStockDraftChange(product._id, "reduce", e.target.value)}
                                                    placeholder="0"
                                                  />
                                                  <button
                                                    type="button"
                                                    ...
                                                    disabled={savingThisProduct || !drafts.reduce}
                                                  >Reduce</button>
                                                </div>
                                              </div>
                                            </div>
                                        ...
                                        <div className="inventory-expiry-box mt-6 p-4 rounded-xl border border-muted-v">
                                          <div className="section-title text-xs mb-3">Health & Expiry Tracking</div>
                                          <div className="flex flex-wrap items-center gap-6">
                                            <label className="flex items-center gap-3 cursor-pointer">
                                              <input
                                                ...
                                                onChange={(e) => handleToggleExpiryTracking(product._id, e.target.checked)}
                                              />
                                              <span className="text-sm font-semibold">Enable Tracking</span>
                                            </label>
                                            
                                            {trackingDraft && (
                                              <div className="flex flex-wrap gap-4 animate-in fade-in slide-in-from-left-2">
                                                <div className="stack-3xs">
                                                  <label className="text-xs uppercase font-bold text-muted">Expiry Date</label>
                                                  <input
                                                    type="date"
                                                    className="input input-sm"
                                                    value={expiryDateDraft}
                                                    min={minExpiryDate}
                                                    onChange={(e) => handleExpiryDraftChange(product._id, "date", e.target.value)}
                                                  />
                                                </div>
                                                <div className="stack-3xs">
                                                  <label className="text-xs uppercase font-bold text-muted">Lead Reminder (Days)</label>
                                                  <input
                                                    type="number"
                                                    className="input input-sm w-24"
                                                    placeholder="10"
                                                    value={reminderDraft}
                                                    onChange={(e) => handleExpiryDraftChange(product._id, "reminder", e.target.value)}
                                                  />
                                                </div>
                                              </div>
                                            )}
                                          ...
                                          {product.expireTrackingEnabled && product.expiryDate && (
                                            <div className={`mt-3 p-2 rounded-lg text-xs flex items-center gap-2 ${daysToExpiry !== null && daysToExpiry <= reminderDaysNumber ? "bg-red-500/10 text-red-400" : "bg-blue-500/10 text-blue-400"}`}>
                                          ...
                                          <div className="flex justify-end mt-4">
                                            ...
                                            >Save Expiry Configuration</button>
                                          </div>
                                        </div>

                                        <div className="footer-actions flex gap-3 justify-end mt-6 pt-6 border-t border-muted-v">
                                            <button
                                              type="button"
                                              className="btn btn-outline btn-md"
                                              onClick={() => handleVariantReset(product._id)}
                                              disabled={savingThisProduct || !variantDirty}
                                            >Reset Drafts</button>
                                            <button
                                              type="button"
                                              className="btn btn-primary btn-md"
                                              onClick={() => handleSaveVariantGroup(product._id)}
                                              disabled={savingThisProduct || !variantDirty}
                                            >
                                              {savingThisProduct ? "Persisting Changes..." : "Commit Inventory Changes"}
                                            </button>
                                        </div>
                                      </div>
                                      
                                      <div className="inventory-reorder-container">
                                        <div className="section-title">Procurement Prep</div>
                                        <div className="stack-md p-6 bg-muted-v rounded-2xl h-full border border-muted-v shadow-inner">
                                          <div className="stack-xs">
                                            <label className="text-xs uppercase font-bold text-muted">Desired Restock Quantity</label>
                                            <input
                                              type="number"
                                              ...
                                              onChange={(e) => setReorderDrafts((prev) => ({
                                                ...prev,
                                                [product._id]: {
                                                  ...prev[product._id],
                                                  qty: e.target.value,
                                          ...
                                          <div className="stack-xs">
                                            <label className="text-xs uppercase font-bold text-muted">Supplier Context / Note</label>
                                            <textarea
                                              className="input input-md h-40 resize-none"
                                              placeholder="Specify batch requirements, urgent lead times, or any specific instructions for the supplier..."
                                          ...
                                          <button
                                            type="button"
                                            className="btn btn-primary w-full btn-lg mt-auto shadow-lg shadow-primary/20"
                                            onClick={() => handleAddToReorder(product._id)}
                                            disabled={!reorderDraft.qty}
                                          >
                                            Queue for Reorder
                                          </button>
                                          {!reorderDraft.qty && <p className="text-xxs text-center muted-text mt-2 italic">Define a quantity to enable procurement</p>}
                                        </div>
                                      </div>
                                    </div>
                                  </div>
  ...
  {tab === "order" && activeOrderPanel === "adminCancelled" && (
        ...
        {tab === "customer-discounts" && (
          <div className="stack-lg">
            <React.Suspense fallback={<div className="empty-cell">Loading…</div>}>
              <CustomerDiscountAdminList />
            </React.Suspense>
          </div>
        )}
          ...
          <div className="stack-lg">
             <div className="card">
                <div className="p-4 border-b border-gray-100 flex items-center justify-between">
                   <h2 className="text-xl font-bold">Predictive Forecasting</h2>
                   <div className="flex items-center gap-2">
                     <span className="text-sm text-gray-500">Target Product:</span>
                     <select 
                        className="input max-w-xs" 
                        style={{ padding: '4px 8px', borderRadius: '6px' }}
                        value={inventoryChartProductId} 
                        onChange={(e) => setInventoryChartProductId(e.target.value)}
                     >
                ...
                <div className="p-6">
                  {inventoryChartProductId ? (
                    <ForecastWidget productId={inventoryChartProductId} />
                  ) : (
                    <div className="p-12 text-center text-gray-400 border-2 border-dashed border-gray-100 rounded-xl">
                        Please select a product from the list to see the 4-week sales forecast.
                    </div>
                  )}
                </div>
             </div>
