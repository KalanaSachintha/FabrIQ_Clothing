...
if (!pk) {
  console.error("Missing REACT_APP_STRIPE_PUBLISHABLE_KEY in .env - Stripe payments unavailable");
}
...
export default function CheckoutPage() {
  ...
  useEffect(() => {
    ...
    if (!pk) {
      setError("Online payments are unavailable. Stripe publishable key is not configured.");
      setLoading(false);
      return;
    }
  ...
  if (!stripePromise) {
    return (
      ...
      >
        <div>
          <div>Online payments are disabled.</div>
          <div style={{ fontSize: "14px", marginTop: "10px" }}>
            Please contact support to configure the Stripe publishable key.
          </div>
        </div>
      </div>
    );
