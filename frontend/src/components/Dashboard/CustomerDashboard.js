    ...
    try {
      ...
      if (checkoutPaymentMethod === "Pay Online") {
        ...
        removeMultipleFromCart(
          items.map((i) => (i.lineId || i.variantKey || i.productId || "").toString())
        );
          ...
          <div className="stack-lg">
            <div className="orders-checkout-list">
              {checkoutItems.map((item) => {
                const id = item.lineId || item.variantKey || item.productId || item._id;
                    ...
                    <div>
                      ...
                      <div className="orders-checkout-item__meta">
                        {item.color || item.size ? (
                          <span className="orders-checkout-item__variant">
                            {[item.color, item.size].filter(Boolean).join(" · ")}
                          </span>
                        ) : null}
                        <span>Qty {item.quantity} × {formatLKR(item.price)}</span>
                      </div>
                    </div>
      ...
      <aside className="card dashboard-sidebar">
        ...
        <nav className="dashboard-nav stack-sm">
          ...
          <button
            ...
            className={`sidebar-link ${tab === "orders" && (ordersView === "overview" || ordersView === "history") ? "is-active" : ""
              }`}
          >
            <span className="sidebar-link__icon">🧾</span>
            <span>My orders</span>
        ...
        {tab === "orders" && (
          <div className="stack-lg">
            ...
            <div className="card stack-md">
              ...
              {(ordersMsg || ordersErr) && (
                <div
                  className={`status-banner ${ordersErr ? "status-banner--error" : "status-banner--success"
                    }`}
                >
                  {ordersErr || ordersMsg}
                </div>
        ...
        {tab === "profile" && (
          <div className="stack-lg">
            {(profileMsg || profileErr) && (
              <div
                className={`status-banner ${profileErr ? "status-banner--error" : "status-banner--success"
                  }`}
              >
            ...
            {canDeleteAccount ? (
              <div className="card stack-md">
                <div className="stack-xs">
                  ...
                  <p className="muted-text">
                    Permanently remove your FabrIQ account, including order history and saved
