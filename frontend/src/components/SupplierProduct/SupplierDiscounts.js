...
export default function SupplierDiscounts() {
  ...
  const handleCreate = async (event) => {
    ...
    try {
      ...
      // Notify other components (e.g. admin customer discounts view) about new supplier discount
      try {
        window.dispatchEvent(new CustomEvent('supplier-discounts-changed', { detail: { action: 'created', discount: discountDoc } }));
      } catch (e) {
        // ignore
      }
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      try {
        window.dispatchEvent(new CustomEvent('supplier-discounts-changed', { detail: { action: 'deleted', id } }));
      } catch (e) {
        // ignore
      }
