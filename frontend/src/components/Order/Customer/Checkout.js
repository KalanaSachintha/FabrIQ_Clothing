...
// Fallback ensures Stripe still works in dev even if env injection fails
const STRIPE_PK_FALLBACK =
  process.env.REACT_APP_STRIPE_PUBLISHABLE_KEY ||
  "pk_test_51SvvPy93OsYd4rvYL2OyVpIeQYNKHOiT6CqSnt3G2oI9CUgVURhiuXtqfVSFh1pN9sgkDLTBYNvSylEbWI9hPgFZ00rz7gKTVi";

const stripePromise = STRIPE_PK_FALLBACK ? loadStripe(STRIPE_PK_FALLBACK) : null;
  ...
  const handlePlaceOrder = async () => {
    if (!validate()) return;
      ...
      const items = selectedItems.map((item) => ({
        ...
        lineId: item.lineId || item.variantKey || null,
      }));
    ...
    try {
      ...
      if (paymentMethod === "Pay Online") {
        ...
        const paymentRes = await fetch(
          `${API_ROOT}/api/payments/stripe/create-checkout-session`,
          {
            method: "POST",
            headers: {
        ...
        const { error } = paymentJson;

        if (!stripePromise) {
          alert("Online payments are temporarily unavailable. Please choose Pay Later.");
          return;
        }
        ...
        if (!stripe) {
          alert("Failed to load Stripe. Please try again later or use Pay Later.");
          return;
        }
        ...
        const orderRes = await fetch(`${API_ROOT}/api/orders`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
        ...
        selectedItems.forEach((item) =>
          removeFromCart((item.lineId || item.variantKey || item.productId || "").toString())
        );
  ...
  return (
    <div className="checkout-page">
      ...
      {selectedItems.map((item) => (
        <p key={item.lineId || item.productId}>
          {item.name}
          {(item.color || item.size) &&
            ` (${[item.color, item.size].filter(Boolean).join(" · ")})`}
          {" "}x {item.quantity} = {formatLKR(item.price * item.quantity)}
        </p>
      ))}
