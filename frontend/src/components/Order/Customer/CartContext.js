...
const CART_STORAGE_PREFIX = "fabriq_cart:";
...
const sanitizeCartItems = (items = []) => {
  ...
  items.forEach((raw) => {
    ...
    const rawProductId = raw.productId || raw._id || raw.id;
    const productId =
      typeof rawProductId === "string" ? rawProductId : rawProductId?.toString();
    if (!productId) return;
    const quantity = normalizeQuantity(raw.quantity);
    const price = Number.isFinite(Number(raw.price)) ? Number(raw.price) : 0;
    const color = normalizeVariantValue(raw.color || raw.selectedColor || raw.colorName);
    const size = normalizeVariantValue(raw.size || raw.selectedSize || raw.sizeLabel);
    const variantKey =
      normalizeKeyValue(raw.variantKey || raw.lineId || raw.cartLineId) ||
      buildVariantKey(productId, color, size);
    ...
    if (map.has(variantKey)) {
      map.set(variantKey, { ...map.get(variantKey), ...entry });
    } else {
      map.set(variantKey, entry);
    }
  });
...
export const CartProvider = ({ children }) => {
  ...
  }, [storageKey, userKey, guestKey, userId, getToken]);
  ...
  }, [cartItems, storageKey, isInitialized, userId, getToken]);
  ...
  const normalizeProduct = useCallback((product, quantity = 1) => {
    ...
    const rawProductId = product.productId || product._id || product.id;
    const productId =
      typeof rawProductId === "string" ? rawProductId : rawProductId?.toString();
    ...
    const color =
      normalizeVariantValue(
        product.selectedColor ||
          product.color ||
          product.colorName ||
          product.preferredColour ||
          product.preferredColor
      ) || null;
    const size =
      normalizeVariantValue(product.selectedSize || product.size || product.sizeLabel) || null;
    const variantKey =
      normalizeKeyValue(product.variantKey || product.lineId || product.cartLineId) ||
      buildVariantKey(productId, color, size);
  ...
  const addToCart = useCallback(
    (product, qty = 1) => {
      ...
      if (!entry) return;
      const entryKey = resolveItemKey(entry);
      setCartItems((prev) => {
        const index = prev.findIndex((item) => resolveItemKey(item) === entryKey);
        if (index >= 0) {
  ...
  const removeFromCart = useCallback((id) => {
    ...
    setCartItems((prev) => prev.filter((item) => resolveItemKey(item) !== target));
  }, []);

  const removeMultipleFromCart = useCallback((ids = []) => {
    if (!Array.isArray(ids) || ids.length === 0) return;
    const lookup = new Set(ids.filter(Boolean).map((value) => value.toString()));
    setCartItems((prev) => prev.filter((item) => !lookup.has(resolveItemKey(item))));
  }, []);
  ...
  const updateQuantity = useCallback((id, quantity) => {
    ...
    setCartItems((prev) =>
      prev.map((item) =>
        resolveItemKey(item) === target ? { ...item, quantity: normalized } : item
      )
    );
  }, []);
  ...
  const contextValue = useMemo(
    ...
    [cartItems, cartState, addToCart, removeFromCart, removeMultipleFromCart, updateQuantity, clearCart, clearServerCart]
  );
