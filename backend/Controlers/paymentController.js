...
const sanitizePendingItemsAndAdjustStock = async (rawItems) => {
  ...
  for (const [productId, quantity] of productRequests.entries()) {
    ...
    const relatedLines = cartLines.filter((l) => l.productId === productId);
    product.adjustStock(relatedLines);
    await product.save();
    
    adjustments.push({ product, relatedLines });
  }

  for (const line of cartLines) {
    const product = productCache.get(line.productId);
    if (!product) {
      throw createHttpError("Unable to build order items. Please try again.");
    }
    const price = Number(product.price);
...
async function createOrderFromPayment(payment, req) {
  ...
  try {
    ...
    if (stockAdjustments.length) {
      await Promise.all(
        stockAdjustments.map(async ({ product, relatedLines }) => {
          try {
            const restoreLines = relatedLines.map((l) => ({ ...l, quantity: -l.quantity }));
...
exports.recordCheckoutSession = async (req, res) => {
  try {
    ...
    const paymentPayload = {
      paymentName: `Vintora Apparel Order ${metadata.orderId}`,
