	...
	try {
		...
		if (status) {
			...
			if (["accepted", "declined"].includes(normalizedStatus)) {
				const wasHandled = ["accepted", "declined"].includes(refundDoc.status) && refundDoc.decision?.decidedAt;
				
				// Handle inventory return if status becomes accepted
				if (normalizedStatus === "accepted" && !wasHandled) {
					try {
						const refundItem = refundDoc.item;
						if (refundItem && refundItem.productId) {
							const product = await Product.findById(refundItem.productId);
							if (product) {
								// Restore stock: pass negative quantity to add it back
								product.adjustStock([{
									...refundItem,
						...
						console.warn("Failed to restore inventory during refund approval:", invErr);
					}
				}
