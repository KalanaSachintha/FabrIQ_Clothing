...
async function updateSupplierDiscount(req, res) {
  try {
    const { id } = req.params;
    const discount = await SupplierDiscount.findById(id);
    if (!discount) return res.status(404).json({ message: 'Discount offer not found' });

    const requesterIsAdmin = isAdmin(req);
    const ownsDiscount = normalizeId(discount.supplierId) === normalizeId(req.user?._id);
    if (!requesterIsAdmin && !ownsDiscount) return res.status(403).json({ message: 'Forbidden' });

    const { discountPercent: dpRaw, discountAmount, minQuantity, note } = req.body || {};
    const parsedDiscount = buildNumber(typeof dpRaw !== 'undefined' ? dpRaw : discountAmount);
    if (typeof dpRaw !== 'undefined' || typeof discountAmount !== 'undefined') {
      if (!Number.isFinite(parsedDiscount) || parsedDiscount <= 0 || parsedDiscount > 100) {
        return res.status(400).json({ message: 'Enter a valid discount percentage (0-100)' });
      }
      discount.discountPercent = parsedDiscount;
    }

    if (typeof minQuantity !== 'undefined') {
      const parsedMin = Math.floor(buildNumber(minQuantity));
      if (!Number.isFinite(parsedMin) || parsedMin <= 0) {
        return res.status(400).json({ message: 'Enter a valid minimum quantity' });
      }
      discount.minQuantity = parsedMin;
    }

    if (typeof note !== 'undefined') discount.note = (note || '').trim() || undefined;

    await discount.save();
    await discount.populate([{ path: 'productId', select: 'name price' }, { path: 'supplierId', select: 'name email' }]);
    const doc = discount.toObject({ virtuals: true });
    doc.discountPercent = doc.discountPercent ?? doc.discountAmount;
    return res.status(200).json({ message: 'Discount updated', discount: doc });
  } catch (error) {
    console.error('Error updating supplier discount', error);
    return res.status(500).json({ message: 'Error updating supplier discount' });
  }
}

module.exports.updateSupplierDiscount = updateSupplierDiscount;
